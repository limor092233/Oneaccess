...
- `GetTargetDivisionIdAsync` implementations, by feature:
  ...
  - `UpdateDivisionCommand`, `DeleteDivisionCommand` → the command's own `DivisionId`.
...
- `CreateDivisionCommand` and `AssignUserDivision` / `RevokeUserDivision` are **not** `IDivisionScopedRequest` — they are the mechanism that defines scope, so they stay System-Administrator-only. `userdivision.assign` / `userdivision.revoke` are seeded to `System Administrator` only and, unlike `user.pincode.reset`, are **not** delegable to `Administrator` through `RolePermission` — letting an Administrator grant Division management (including to themselves) would defeat the whole scheme.
