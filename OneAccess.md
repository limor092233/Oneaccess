...
**Non-delegable permissions:** The following permissions are seeded to `System Administrator` only and are **strictly non-delegable** (i.e. `AssignRolePermissionCommandHandler` explicitly rejects assigning them to any role other than System Administrator, returning `400 Bad Request` with `ErrorCode.NonDelegablePermission`):

| Category | Permission Codes | Security Justification / Rationale |
|---|---|---|
| **Division Scoping & Management** | `division.create`<br>`division.update`<br>`division.delete`<br>`userdivision.assign`<br>`userdivision.revoke` | Divisions establish the fundamental tenant/organizational security boundary and scoping mechanism. Allowing non-SysAdmins to create/alter/delete divisions or assign division managers would allow lateral privilege escalation and scope escape across division boundaries. |
| **Sub-System Governance & SSO Access** | `subsystem.register`<br>`subsystem.update`<br>`rolesubsystem.assign`<br>`rolesubsystem.revoke`<br>`usersubsystem.assign`<br>`usersubsystem.revoke` | Sub-systems represent external integrated applications in the SSO topology. Gating registration, URL configuration, and role/user access allow-lists prevents delegated administrators from bypassing portal visibility policies or granting themselves unapproved access to sensitive sub-systems. |
| **Role & Permission Governance** | `role.create`<br>`role.update`<br>`role.delete`<br>`rolepermission.assign`<br>`rolepermission.revoke` | Roles and permission assignments constitute the central RBAC security foundation. If an Administrator could create arbitrary custom roles or assign permissions, they could construct high-privilege roles or grant unassigned capabilities, subverting role isolation. |
| **System-Wide Audit Trail** | `audit.view` | `AuditLog` records global authentication and administrative lifecycle events across all divisions without division scoping. Exposing it to delegated administrators would leak cross-division activity and sensitive administrative operations. `GetAuditLogsQuery` deliberately does not implement `IDivisionScopedRequest`. |
